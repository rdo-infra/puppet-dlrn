...
if [ "$TAG_PHASE" = "build" ]; then
    TAG_SUFFIX="el${CENTOS_VERSION}-$TAG_PHASE"
else
...
if [ $RELEASE = "master-uc" ]; then
    CBS_TAG=${CBS_TAG:-"cloud${CENTOS_VERSION}-openstack-${MASTER_TAG}-${TAG_SUFFIX}"}
else
    CBS_TAG=${CBS_TAG:-"cloud${CENTOS_VERSION}-openstack-${RELEASE}-${TAG_SUFFIX}"}
fi
...
if [ $CENTOS_VERSION -eq 8 ]; then
    ...
    timeout 15m $COPR_LIST_SCRIPT $TEMPDIR
    # In CentOS8 we need to clean cached repos metadata for user
    dnf clean all
fi
