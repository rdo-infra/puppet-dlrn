...
if [ -n "${RSYNC_DEST}" ]; then
    ...
    ssh -p ${RSYNC_PORT} -o StrictHostKeyChecking=no \
        $RSYNC_SERVER "find /home/${USER}/data/repos -maxdepth 1 -type l -exec readlink -f {} \; | grep -vE 'current$|consistent$|delorean-deps.repo$'" \
                       | tail -n +2 | sort | uniq | while read symlink; do
