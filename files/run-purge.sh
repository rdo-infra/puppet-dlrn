...
if [ -n "${RSYNC_DEST}" ] && [[ $USE_COMPONENTS =~ ^[Tt]rue$ ]]; then
    ...
    ssh -p ${RSYNC_PORT} -o StrictHostKeyChecking=no $RSYNC_SERVER "find /home/${USER}/data/repos/component -maxdepth 2 -type l 2>/dev/null| grep -vE 'current$|consistent$'" | sort | uniq | while read symlink; do
    ...
    ssh -p ${RSYNC_PORT} -o StrictHostKeyChecking=no $RSYNC_SERVER "find /home/${USER}/data/repos -maxdepth 1 | grep -vE /home/${USER}/data/repos/[0-9a-f]{2} | grep -vE '"${BLACKLIST_DIRS}"' " | sort | uniq | while read directory; do
    ...
    ssh -p ${RSYNC_PORT} -o StrictHostKeyChecking=no $RSYNC_SERVER "find /home/${USER}/data/repos -maxdepth 1 -type l -exec readlink -f {} \; | grep -vE 'current$|consistent$|delorean-deps.repo$'" | sort | uniq | while read symlink; do
