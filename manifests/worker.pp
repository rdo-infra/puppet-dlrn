...
# [*downstream_prefix*]
#   (optional) If pkginfo_driver is
#   'dlrn.drivers.downstream.DownstreamInfoDriver', this option, when set to
#   a value like 'foo-', will cause the downstream driver to look at the
#   foo-distgit attribute instead of just distgit.
#   Defaults to undef.
#
# [*downstream_distro_filter*]
#   (optional) If pkginfo_driver is
#   'dlrn.drivers.downstream.DownstreamInfoDriver', this option specifies
#   whether the packages section of packaging metadata (from distroinfo) should
#   be filtered to only contain packages with downstream_prefix-ed attributes.
#   Defaults to false.
#
# [*koji_exe*]
...
define dlrn::worker (
  ...
  $downstream_prefix             = undef,
  $downstream_distro_filter      = false,
  $koji_exe                      = 'koji',
